...
function mapgen()
 ...
 --decorations
	rooms={}
	roommap=blankmap(0)--will track whether a given tile belongs to a room
	doors={}
 genrooms()
 mazeworm()
 placeflags()
...
function placeroom(r)
 ...
 r.x=c.x
 r.y=c.y
	add(rooms,r)--gives us the dimensions and locations of all rooms  
 for _x=0,r.w-1 do
  for _y=0,r.h-1 do
...
--makes sure we aren't make a tunnel
...
--[[have to be careful of a memory
leak here, where the cand array
can grow and grow if you don't
stick it straight into candnew
once identified
--]] 
...
--reverse worm. undoes dead ends that are where the worm carved to far, and a door was never punched out, for whatever raise
--a dead end is a tile that has only one cardinal exit. only one way to reach it. a tunnel has two exits. 3 or 4, crossroads or room
--since it repeats until, it's going to 
...
function isdoor(x,y)
 for i=1,4 do
  if inbounds(x+dirx[i],y+diry[i]) and roommap[x+dirx[i]][y+diry[i]]!=0 then
   return true
  end
 end
 return false
...
--[[
	find a random spot for a starting point
	then build a distmap, then pick the furthest point
	then finding a good position for the stairs in those locations
	ideally in an alcove, use the worm function to dig it
--]]

function startend()
	--when placing the first stair, it's going 
	--to look for the farthest spot
	--from a random spot, guarantees
	--it will be kind of tucked away
	--need to actually shape it so that
	--it ends up in an alcove, so we
	--reuse the can carve an alcove, so
	--the stairs down never block progress
	--that's why there is cancarve in the cond
	--px,py is start location, ex, ey is exit location
	--high is the temporarily highest known part of the map
	--to be checked against
	--add signature so that entr is always
...
__gff__

__map__

